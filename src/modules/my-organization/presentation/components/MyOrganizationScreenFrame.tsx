...
import {
  ...
  type MyOrganizationStackParamList,
} from '@/navigation/route-types';
import { useAppSelector } from '@/store/hooks';
import { selectIsAuthenticated } from '@/store/slices/authSlice';
import { fontSize, spacing } from '@/utils/scale';

type MyOrganizationNavigation = NavigationProp<MyOrganizationStackParamList>;
const moduleColors = reactNativeColorScheme.ultiHuman.module;

interface MyOrganizationScreenFrameProps {
  isDetail?: boolean;
  navigation: MyOrganizationNavigation;
  onRefresh?: () => void;
  readOnlyNote?: string;
  refreshing?: boolean;
  routeLabel?: string;
...
export const MyOrganizationScreenFrame = ({
  children,
  isDetail = false,
  navigation,
  onRefresh,
  readOnlyNote,
  refreshing = false,
  routeLabel = 'My Organization',
  title,
}: PropsWithChildren<MyOrganizationScreenFrameProps>) => {
  ...
  return (
    ...
    >
      ...
      {readOnlyNote ? <Text style={styles.readOnlyNote}>{readOnlyNote}</Text> : null}
    </MobileScreenShell>
  );
};
